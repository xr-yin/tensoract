"""Test the fast disentangling algorithm from https://arxiv.org/pdf/2104.08283"""
...
def fastDisentangle(chi1, chi2, A, transposeQ=None):
    ...
    V3  = np.asmatrix(np.linalg.svd(np.tensordot(A, alpha4,   1  ), full_matrices=False)[2][:chi1]).H # 3
    V4  = np.asmatrix(np.linalg.svd(np.tensordot(A, alpha3, (1,0)), full_matrices=False)[2][:chi2]).H # 4
    ...
    while not allOrthog:
        ...
        for i in range(m):
            if not orthogQ[i]:
                ...
                allOrthog = False
        eps = eps*eps
    if reorthogonalizeQ and np.linalg.norm(M * np.asmatrix(M).H - np.eye(m)) > np.finfo(M.dtype).eps ** 0.75:
